...
const applyDisposition = async (journal, issue) => {
  ...
  if (selected.implementationTicket) {
    ...
    if (ticket.runtimeEvidence !== undefined) {
      if (ticket.issueNumber === undefined || runtimeEvidenceHandoff === undefined) {
        throw new Error("Implementation ticket runtime-evidence contract has no assigned issue number; resume the ticket creation boundary");
      }
    ...
    const laterArtifactRecovery = (() => {
      ...
      try {
        commits = hostGit(["rev-list", "--ancestry-path", start + ".." + candidate], { encoding: "utf8" })
          .replaceAll(String.fromCharCode(13), "").trim().split(String.fromCharCode(10)).filter(Boolean).reverse();
      } catch {
        return undefined;
      }
    ...
    if (pendingPhases.length > 0 || evidenceArtifactPending) {
      ...
      const phaseCallbacks = {
        ...
        onPhaseComplete: async (phaseResult) => {
          ...
          // The agent is allowed to create the program-produced image.  The
          // host-owned screenshot boundary later validates its runtime
          // assertion and permits only the exact artifact commit, which is
          // also required for crash-safe recovery.
      ...
      const runBatch = async (batchSandbox, batch, includeSetup = false) => {
        ...
        return await retrySequential(() => runWorkflow({
          ...
          retryable: (error) => isTransientSequentialError(error) ||
            (batch.some((phase) => phase.type === "agent") &&
              typeof isRetryableCodexProviderError === "function" && isRetryableCodexProviderError(error)),
        });
      };
