              ...
              (let* ((out #$output)
                ...
                (call-with-output-file runner
                  (lambda (port)
                    ...
                    (display "            and b'space to continue' in lower[confirm_output_start:]):\n" port)
                    (display "        os.write(master, b' ')\n" port)
                    (display "        sent_continue = True\n" port)
                    (display "        continue_output_start = len(seen)\n" port)
                    (display "    if (sent_continue and not sent_record\n" port)
                    (display "            and continue_output_start is not None\n" port)
                    (display "            and all(word in lower[continue_output_start:]\n" port)
