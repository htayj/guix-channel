              ...
              (let* ((out #$output)
                ...
                (call-with-output-file runner
                  (lambda (port)
                    ...
                    (display "            and all(word in lower for word in (b'quit', b'abandon', b'game'))):\n" port)
                    (display "        os.write(master, b'y')\n" port)
                    (display "        sent_confirm = True\n" port)
                    (display "    if (sent_confirm and not sent_record\n" port)
                    (display "            and all(word in lower for word in (b'save', b'recording', b'as'))):\n" port)
                ...
                (call-with-output-file launcher
                  (lambda (port)
                    ...
                    (display "  \"$python\" \"$runner\" \"$program\" -t -s 1 -n --data-dir \"$data\" >\"$scratch/ui.raw\"\n" port)
