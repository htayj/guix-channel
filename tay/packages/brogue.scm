              ...
              (let* ((out #$output)
                ...
                (call-with-output-file runner
                  (lambda (port)
                    ...
                    (display "    ready, _, _ = select.select([master], [], [], " port)
                    (display "0.2)\n" port)
                    (display "    if ready:\n" port)
                    (display "        try:\n            chunk = os.read(master, " port)
                    (display "8192)\n" port)
                    ...
                    (display "            evidence.extend(chunk[:65536 - " port)
                    (display "len(evidence)])\n" port)
                    ...
                    (display "    if (not sent_move and " port)
                    (display "(b'hp' in lower or b'health' in lower)\n" port)
                    (display "            and (b'depth' in lower or " port)
                    (display "b'level' in lower)):\n" port)
                    ...
                    (display "            and b'space to continue' in " port)
                    (display "visible_lower):\n" port)
                    (display "        os.write(master, b' ')\n" port)
                    (display "        sent_continue = True\n" port)
                    (display "    if (sent_continue and not sent_record\n" port)
                    (display "            and b'save recording as' in " port)
                    (display "visible_lower):\n" port)
                    ...
                    (display "        menu_seen_new = menu_seen_new or " port)
                    (display "b'new game' in visible_lower\n" port)
                    (display "        menu_seen_quit = menu_seen_quit or " port)
                    (display "b'quit' in visible_lower\n" port)
                    ...
                    (display "    if finished:\n        reaped = True\n" port)
                    (display "        break\n\n" port)
                    (display "if not reaped:\n    _, status = os.waitpid(" port)
                    (display "pid, 0)\n" port)
                    (display "os.close(master)\n" port)
                    (display "if not (sent_move and sent_quit and " port)
                    (display "sent_confirm\n" port)
                    (display "        and sent_continue\n" port)
                    (display "        and sent_record and sent_final\n" port)
                    (display "        and sent_menu_quit):\n" port)
                    (display "    raise SystemExit('Brogue PTY smoke missed an " port)
                    (display "input state')\n" port)
                    (display "if not os.WIFEXITED(status) or " port)
                    (display "os.WEXITSTATUS(status):\n" port)
                    (display "    raise SystemExit('Brogue exited " port)
                    (display "unsuccessfully')\n" port)
                    (display "text = (bytes(evidence) + bytes(seen)).decode(" port)
                    (display "'utf-8', 'replace')\n" port)
                    (display "assert 'Dungeons of Doom' in text, " port)
                    (display "text[-4096:]\n" port)
                    (display "assert ('HP' in text or 'Health' in text), " port)
                    (display "text[-4096:]\n" port)
                    (display "assert ('Depth' in text or 'Level' in text), " port)
                    (display "text[-4096:]\n" port)
                    (display "assert '@' in text, text[-4096:]\n" port)
                    (display "recordings = list(Path.cwd().rglob(" port)
                    (display "'*.broguerec'))\n" port)
                    (display "assert recordings, 'no recording was " port)
                    (display "created'\n" port)
                    (display "root = Path.cwd().resolve()\n" port)
                    (display "assert all(path.resolve().is_relative_to(root)\n" port)
                    (display "               for path in recordings), " port)
                    (display "recordings\n" port)))
                (chmod runner #o555)
                (call-with-output-file launcher
                  (lambda (port)
                    ...
                    (display "if test \"${1:-}\" = --guix-smoke && " port)
                    (display "test \"$#\" -eq 1; then\n" port)
                    (display "  scratch=$(\"$mktemp\" -d \"${TMPDIR:-/tmp}/" port)
                    (display "brogue-smoke.XXXXXXXX\")\n" port)
                    (display "  \"$mkdir\" -p \"$scratch/home\" " port)
                    (display "\"$scratch/config\" \"$scratch/cache\"\n" port)
                    (display "  \"$mkdir\" -p \"$scratch/data\" \"$scratch/state\" " port)
                    (display "\"$scratch/runtime\" \"$scratch/tmp\"\n" port)
                    ...
                    (display "  export TMPDIR=\"$scratch/tmp\" TERM=xterm-256color " port)
                    (display "LC_ALL=C\n" port)
                    (display "  \"$mkdir\" -p \"$XDG_STATE_HOME/brogue\"\n" port)
                    (display "  \"$cp\" \"$keymap\" \"$XDG_STATE_HOME/brogue/" port)
                    (display "keymap.txt\"\n" port)
                    (display "  cd \"$XDG_STATE_HOME/brogue\"\n" port)
                    (display "  if ! \"$python\" \"$runner\" \"$program\" " port)
                    (display "-t -s 1 -n --data-dir \"$data\" >\"$scratch/ui.raw\"; then\n" port)
